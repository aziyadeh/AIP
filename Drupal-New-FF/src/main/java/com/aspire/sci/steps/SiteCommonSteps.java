...
public class SiteCommonSteps {
	...
	@Given("[8101-0026] User check if cookies tab display")
	@When("[8101-0026] User check if cookies tab display")
	@Then("[8101-0026] User check if cookies tab display")
	public void cookiesTab() {
		AspireWebElement x = AspireBrowser.getElementByPropertyNameGlobaly("cookie");
		try {
			if(x.isDisplayed())
			{
				x.click();
